...
const cherryPickPR = async (
  ...
  try {
    ...
    await execa("git", ["diff", originCommand, "--", "Pipfile", "package.json", ">", "custom.patch"])
    const {stdout} = await execa("cat", ["custom.patch"])
    logger.info(stdout)
    const {checkError} = await execa("git", ["apply", "-C0", "custom.patch"])
    logger.info(checkError);
    if (checkError.includes('error: patch failed')){
      return false
    }
    return true
  } catch (err) {
